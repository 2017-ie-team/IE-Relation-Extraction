...
print('In terms of frequency and set')
NAME = set()
for elem in [Entityset,EntityDetectset,EntityCorrectset]:
	for item in elem:
		NAME.add(item)
EntityFr = mydict(int)
EntityCorrectFr = mydict(int)
for naming in NAME:
	EntityFr[naming] = len(Entitylist[naming])
	for ele in EntityCorrectset[naming]:
		EntityCorrectFr[naming] += Entitylist[naming].count(ele)
print(EntityFr)
print(EntityCorrectFr)
